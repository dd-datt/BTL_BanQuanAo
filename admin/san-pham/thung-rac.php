<?php
// Khởi tạo biến thông báo thành công
$success = '';

// Xử lý khi người dùng yêu cầu xóa tạm thời sản phẩm
if (isset($_GET['xoatam']) && $_GET['xoatam'] > 0) {
    $product_id = $_GET['xoatam'];

    // Cập nhật trạng thái không hoạt động của sản phẩm
    $ProductModel->update_product_not_active($product_id);
    $success = '1 sản phẩm đã thêm vào thùng rác';
}

// Xử lý khi người dùng yêu cầu khôi phục sản phẩm
...
// Xử lý khi người dùng yêu cầu xóa vĩnh viễn sản phẩm
if (isset($_GET['xoa'])) {
    $product_id = $_GET['xoa'];
...
// Tạo HTML thông báo thành công hoặc thất bại
